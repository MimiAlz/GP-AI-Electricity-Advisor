...
# ─────────────────────────────────────────────────────────────────────────────
# 0.  Checkpoint helpers
# ─────────────────────────────────────────────────────────────────────────────

def _load_state_dict_from_dir(dir_path: str, device: torch.device) -> dict:
    """
    Load a PyTorch state dict from an unpacked ZIP directory.

    When torch.save() archives are extracted (e.g. via unzip), the internal
    archive/ contents land at the top level of the directory.  This function
    re-assembles them into an in-memory BytesIO ZIP so torch.load() can read
    them normally.

    The resulting ZIP entry names are prefixed with 'archive/' to match the
    layout PyTorch expects.
    """
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, 'w', zipfile.ZIP_STORED, strict_timestamps=False) as zf:
        for root, _dirs, files in os.walk(dir_path):
            for fname in sorted(files):
                if fname == '.DS_Store':
                    continue
                full = os.path.join(root, fname)
                arcname = 'archive/' + os.path.relpath(full, dir_path)
                zf.write(full, arcname)
    buf.seek(0)
    return torch.load(buf, map_location=device, weights_only=False)


def _resolve_checkpoint(checkpoint_dir: str, meta_file: str, raw_ckpt_field: str) -> Optional[str]:
    """
    Locate the actual checkpoint for an appliance.

    Search order:
      1. Unpacked directory named after the stem of raw_ckpt_field  (new layout)
      2. .pt file with that same stem inside checkpoint_dir         (legacy layout)
      3. raw_ckpt_field as an absolute or relative path             (fallback)

    Returns the resolved path/dir string, or None if nothing found.
    """
    stem = os.path.splitext(os.path.basename(raw_ckpt_field))[0]  # e.g. best_TV_v4

    dir_candidate = os.path.join(checkpoint_dir, stem)
    if os.path.isdir(dir_candidate):
        return dir_candidate

    pt_candidate = os.path.join(checkpoint_dir, stem + '.pt')
    if os.path.isfile(pt_candidate):
        return pt_candidate

    if os.path.exists(raw_ckpt_field):
        return raw_ckpt_field
...
class NILMBackend:
    ...
    def from_checkpoints(
        ...
        for mf in sorted(meta_files):
            ...
            app  = raw['appliance']
            ckpt = _resolve_checkpoint(checkpoint_dir, mf, raw.get('checkpoint', ''))

            if ckpt is None:
                print(f'  [WARNING] Checkpoint missing for {app} — skipping')
                continue
            ...
            if os.path.isdir(ckpt):
                state_dict = _load_state_dict_from_dir(ckpt, device)
            else:
